...
.NOTES
    ...
    Version: 2.1
    Changes in v2.1: Added service re-registration to ensure poll interval is properly applied
#>
...
function Test-NtpServer {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Log "Could not reach $Server (will retry during sync)" -Level Warning
        }
    }
    catch {
...
# Main execution
try {
    Write-Log "=== Windows NTP Configuration Script v2.1 ===" -Level Info
    ...
    # Stop service first
    Write-Log "`nStopping Windows Time service..." -Level Info
    ...
    # Unregister and re-register to ensure clean configuration reload
    Write-Log "Re-registering Windows Time service..." -Level Info
    $unregResult = w32tm /unregister 2>&1
    if ($LASTEXITCODE -ne 0) {
        Write-Log "Unregister returned non-zero code (may be normal): $LASTEXITCODE" -Level Warning
    }
    ...
    # NOW configure registry (after register resets defaults)
    Write-Log "Configuring W32Time parameters..." -Level Info
    Set-ItemProperty -Path $w32timeParams -Name "NtpServer" -Value $NtpServers -ErrorAction Stop
    Set-ItemProperty -Path $w32timeParams -Name "Type" -Value "NTP" -ErrorAction Stop
    Write-Log "NTP servers configured" -Level Success
    
    # Configure NtpClient Provider
    ...
    Set-ItemProperty -Path $ntpClientPath -Name "SpecialPollInterval" -Value $SpecialPollInterval -Type DWord -ErrorAction Stop
    Set-ItemProperty -Path $ntpClientPath -Name "Enabled" -Value 1 -Type DWord -ErrorAction Stop
    Write-Log "Poll interval configured" -Level Success
    
    # Configure additional reliability settings
    Write-Log "Configuring reliability settings..." -Level Info
    $configPath = "HKLM:\SYSTEM\CurrentControlSet\Services\W32Time\Config"
    Set-ItemProperty -Path $configPath -Name "MaxPosPhaseCorrection" -Value 3600 -Type DWord -ErrorAction Stop
    Set-ItemProperty -Path $configPath -Name "MaxNegPhaseCorrection" -Value 3600 -Type DWord -ErrorAction Stop
    Set-ItemProperty -Path $configPath -Name "UpdateInterval" -Value 100 -Type DWord -ErrorAction Stop
    Write-Log "Reliability settings applied" -Level Success
    
    # Set service to Automatic startup
    Write-Log "Setting service startup type..." -Level Info
    Set-Service -Name w32time -StartupType Automatic -ErrorAction Stop
    Write-Log "Service set to Automatic startup" -Level Success
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Log "w32tm /config /update returned code: $LASTEXITCODE" -Level Warning
    }
    else {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Log "Resync returned code $LASTEXITCODE (sync may still be in progress)" -Level Warning
    }
